...
async def disconnect_all_and_reconnect(server: ChivesServer, reconnect_to: ChivesServer) -> bool:
...
async def add_dummy_connection(
    server: ChivesServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
...
async def connect_and_get_peer(server_1: ChivesServer, server_2: ChivesServer) -> WSJokerConnection:
    """

...
from joker.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from joker.full_node.full_node_api import FullNodeAPI
from joker.server.start_farmer import service_kwargs_for_farmer
from joker.server.start_full_node import service_kwargs_for_full_node
from joker.server.start_harvester import service_kwargs_for_harvester
from joker.server.start_introducer import service_kwargs_for_introducer
from joker.server.start_service import Service
from joker.server.start_timelord import service_kwargs_for_timelord
from joker.server.start_wallet import service_kwargs_for_wallet
from joker.simulator.start_simulator import service_kwargs_for_full_node_simulator
from joker.timelord.timelord_launcher import kill_processes, spawn_process
...
from joker.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["xjk_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xch")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xjk_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xch")

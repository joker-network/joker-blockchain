...
#from joker.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from joker.plotters.chiapos import get_chiapos_install_info, plot_joker
from joker.plotters.madmax import get_madmax_install_info, plot_madmax
from joker.plotters.install_plotter import install_plotter
...
#            )
        ...
        if option is Options.OVERRIDE_K:
            parser.add_argument(
                ...
                dest="override",
                action="store_true",
                help="Force size smaller than 30",
                default=False,
            )
        ...
        if option is Options.MADMAX_PORT:
            parser.add_argument(
                ...
                "--port",
                type=int,
                help="Specifies the port number necessary for Joker plot generation",
                default=18444,
            )


def call_plotters(root_path: Path, args):
    # Add `plotters` section in JOKER_ROOT.
    joker_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within JOKER_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    if args.plotter == "chiapos":
        plot_joker(args, joker_root_path)

...
from joker.protocols.protocol_state_machine import message_requires_reply
from joker.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from joker.protocols.shared_protocol import protocol_version
from joker.server.introducer_peers import IntroducerPeers
from joker.server.outbound_message import Message, NodeType
from joker.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from joker.util.errors import Err, ProtocolError
from joker.util.ints import uint16
from joker.util.network import is_localhost, is_in_network
from joker.util.ssl_check import verify_ssl_certs_and_keys
...
class JokerServer:
    def __init__(
        ...
        self.all_connections: Dict[bytes32, WSJokerConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSJokerConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.joker_ca_crt_path, self.joker_ca_key_path = joker_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSJokerConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.joker_ca_crt_path, self.joker_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSJokerConnection] = None
        try:
            connection = WSJokerConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSJokerConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.joker_ca_crt_path, self.joker_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSJokerConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSJokerConnection, ban_time: int):
    ...
    def get_outgoing_connections(self) -> List[WSJokerConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSJokerConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSJokerConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSJokerConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use joker's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.jokercoin.org/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSJokerConnection, trusted_peers: Dict) -> bool:

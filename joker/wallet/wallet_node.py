...
from joker.daemon.keychain_proxy import (
    KeychainProxy,
...
from joker.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from joker.protocols import wallet_protocol
from joker.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from joker.server.node_discovery import WalletPeers
...
from joker.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, load_config
from joker.util.errors import Err, ValidationError
from joker.util.ints import uint32, uint128
from joker.util.keychain import Keychain
from joker.util.lru_cache import LRUCache
from joker.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from joker.util.network import get_host_addr
from joker.util.path import mkdir, path_from_root
from joker.wallet.block_record import HeaderBlockRecord
from joker.wallet.derivation_record import DerivationRecord
from joker.wallet.settings.settings_objects import BackupInitialized
from joker.wallet.transaction_record import TransactionRecord
from joker.wallet.util.backup_utils import open_backup_file
from joker.wallet.util.wallet_types import WalletType
from joker.wallet.wallet_action import WalletAction
from joker.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    def __init__(
        ...
        self.short_sync_threshold = 1  # Change the test when changing this
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'joker keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSJokerConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSJokerConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSJokerConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSJokerConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSJokerConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSJokerConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSJokerConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:

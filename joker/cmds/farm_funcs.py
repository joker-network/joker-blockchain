...
from joker.rpc.farmer_rpc_client import FarmerRpcClient
from joker.rpc.full_node_rpc_client import FullNodeRpcClient
from joker.rpc.wallet_rpc_client import WalletRpcClient
...
from joker.util.misc import format_bytes
from joker.util.misc import format_minutes
from joker.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total joker farmed: {amounts['farmed_amount'] / units['joker']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['joker']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['community_reward_amount'] + amounts['pool_reward_amount']) / units['joker']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'joker start wallet' and 'joker wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'joker wallet show'")
    else:
        print("Note: log into your key using 'joker wallet show' to see rewards for each key")

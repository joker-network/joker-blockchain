...
def show_plots(root_path: Path):
    from joker.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'joker plots add -d [dir]' and remove with"
        + " 'joker plots remove -d [dir]'"
        + " Scan and check plots with 'joker plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'joker init'")
...
def create_cmd(
    ...
    from joker.plotting.create_plots import create_plots, resolve_plot_keys
    ...
    if size < 30 and not override_k:
        print("k=30 is the minimum size for farming.")
...
def check_cmd(
    ...
    from joker.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from joker.plotting.util import remove_plot_directory

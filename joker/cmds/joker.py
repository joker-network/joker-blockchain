...
from joker.cmds.configure import configure_cmd
from joker.cmds.farm import farm_cmd
from joker.cmds.init import init_cmd
from joker.cmds.keys import keys_cmd
from joker.cmds.netspace import netspace_cmd
from joker.cmds.passphrase import passphrase_cmd
from joker.cmds.plots import plots_cmd
from joker.cmds.show import show_cmd
...
@click.group(
    help=f"\n  Manage joker blockchain infrastructure ({__version__})\n",
    epilog="Try 'joker start node', 'joker netspace -d 192', or 'joker show -s'",
...
@cli.command("version", short_help="Show joker version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs joker daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by joker start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from joker.daemon.server import async_run_daemon

...
from joker.consensus.coinbase import create_puzzlehash_for_pk
from joker.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_joker_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from joker.util.bech32m import encode_puzzle_hash
from joker.util.config import (
    create_default_joker_config,
    initial_config_file,
    load_config,
    save_config,
...
from joker.util.ssl_check import (
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'joker keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xjk_target_address" not in config["farmer"]
    stop_searching_for_pool = "xjk_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xjk_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xjk_target_address"):
    ...
    if "xjk_target_address" not in config["farmer"]:
        ...
        updated_target = True
    elif config["farmer"]["xjk_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xjk_target_address']} with {all_targets[0]}"
        )
    ...
    if "xjk_target_address" not in config["pool"]:
        ...
        print(
            ...
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xjk_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the XJK destination addresses, edit the `xjk_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    joker_ca_crt, joker_ca_key = get_joker_ca_crt_key()
    joker_ca_crt_path = ca_dir / "joker_ca.crt"
    joker_ca_key_path = ca_dir / "joker_ca.key"
    write_ssl_cert_and_key(joker_ca_crt_path, joker_ca_crt, joker_ca_key_path, joker_ca_key)
    ...
    joker_ca_crt, joker_ca_key = get_joker_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, joker_ca_crt, joker_ca_key, False, overwrite=False)
...
def joker_minor_release_number():
    res = int(joker_version_number()[2])
    print(f"Install release number: {res}")
    return res


def joker_full_version_str() -> str:
    major, minor, patch, dev = joker_version_number()
...
def joker_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    ...
    """
    if os.environ.get("JOKER_ROOT", None) is not None:
        print(
            f"warning, your JOKER_ROOT is set to {os.environ['JOKER_ROOT']}. "
            f"Please unset the environment variable and run joker init again\n"
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'joker keys show --show-mnemonic-seed'")

...
ROOT = pathlib.Path(importlib.import_module("joker").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `joker` file name (which is the executable)
    and the `joker` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `joker` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("joker/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("joker-blockchain"))[0]
...
# TODO: collapse all these entry points into one `joker_exec` entrypoint that accepts the server as a parameter

entry_points = ["joker.cmds.joker"] + [f"joker.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "joker.util.bip39"])

if THIS_IS_WINDOWS:
    joker_mod = importlib.import_module("joker")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/joker/util/english.txt", "joker/util"))
datas.append((f"{ROOT}/joker/util/initial-config.yaml", "joker/util"))
datas.append((f"{ROOT}/joker/wallet/puzzles/*.hex", "joker/wallet/puzzles"))
datas.append((f"{ROOT}/joker/ssl/*", "joker/ssl"))
...
add_binary("joker", f"{ROOT}/joker/cmds/joker.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/joker/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/joker/server/start_{server}.py", COLLECT_ARGS)

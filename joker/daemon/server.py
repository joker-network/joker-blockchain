...
from joker.cmds.init_funcs import check_keys, joker_init
from joker.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from joker.daemon.keychain_server import KeychainServer, keychain_commands
from joker.daemon.windows_signal import kill
from joker.plotters.plotters import get_available_plotters
from joker.plotting.util import add_plot_directory
from joker.server.server import ssl_context_for_root, ssl_context_for_server
from joker.ssl.create_ssl import get_mozilla_ca_crt
from joker.util.joker_logging import initialize_logging
from joker.util.config import load_config
from joker.util.json_util import dict_to_json_str
from joker.util.keychain import (
    Keychain,
...
from joker.util.service_groups import validate_service
from joker.util.setproctitle import setproctitle
from joker.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Joker.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "joker": "joker",
        "joker_wallet": "start_wallet",
        "joker_full_node": "start_full_node",
        "joker_harvester": "start_harvester",
        "joker_farmer": "start_farmer",
        "joker_introducer": "start_introducer",
        "joker_timelord": "start_timelord",
        "joker_timelord_launcher": "timelord_launcher",
...
#        return command_args
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["joker", "plotters", plotter]
...
#            command_args.extend(self._bladebit_plotting_command_args(request, ignoreCount))
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("joker daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered JOKER_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with JOKER_ROOT: {os.environ['JOKER_ROOT']}")
    ...
    if service_command == "joker_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in joker_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    joker_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("joker_daemon")

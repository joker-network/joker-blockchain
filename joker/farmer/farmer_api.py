...
from joker.protocols.harvester_protocol import PoolDifficulty
...
from joker.util.api_decorators import api_request, peer_required
...
class FarmerAPI:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Joker Blockchain v.{__version__}"},
                        ) as resp:
                            if resp.ok:
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSJokerConnection):

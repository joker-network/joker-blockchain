...
from joker.pools.pool_config import PoolWalletConfig, load_pool_config
from joker.protocols import farmer_protocol, harvester_protocol
from joker.protocols.pool_protocol import (
    ErrorResponse,
...
from joker.server.outbound_message import NodeType, make_msg
from joker.server.server import ssl_context_for_root
...
from joker.util.config import load_config, save_config, config_path_for_filename
from joker.util.hash import std_hash
from joker.util.ints import uint8, uint16, uint32, uint64
...
from joker.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self):
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'joker keys generate' or open the UI."
            raise RuntimeError(error_str)

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["xjk_target_address"]
        ...
        self.pool_target_encoded = self.pool_config["xjk_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        ...
        if farmer_target_encoded is not None:
            ...
            config["farmer"]["xjk_target_address"] = farmer_target_encoded
        if pool_target_encoded is not None:
            self.pool_target_encoded = pool_target_encoded
            self.pool_target = decode_puzzle_hash(pool_target_encoded)
            config["pool"]["xjk_target_address"] = pool_target_encoded
    ...
    async def get_cached_harvesters(self, connection: WSJokerConnection) -> HarvesterCacheEntry:
